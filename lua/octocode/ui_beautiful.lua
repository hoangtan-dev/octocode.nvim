...
local function setup_highlighting()
  ...
  vim.cmd([[
    highlight OctocodeInput guifg=#61AFEF gui=bold ctermfg=75 cterm=bold
    highlight OctocodeMode guifg=#98C379 gui=italic ctermfg=114 cterm=italic
    highlight OctocodeSeparator guifg=#5C6370 gui=bold ctermfg=59 cterm=bold
    highlight OctocodeHeader guifg=#E06C75 gui=bold ctermfg=204 cterm=bold
    highlight OctocodeFile guifg=#D19A66 gui=underline ctermfg=173 cterm=underline
    highlight OctocodeSymbol guifg=#C678DD gui=italic ctermfg=176 cterm=italic
    ...
    highlight OctocodeEmoji guifg=#E5C07B gui=bold ctermfg=180 cterm=bold
  ]])
  ...
  if state.search_buf and vim.api.nvim_buf_is_valid(state.search_buf) then
    vim.api.nvim_buf_call(state.search_buf, function()
      vim.cmd([[
        syntax match OctocodeInput /^🔍 Search \[.*\]:/
        syntax match OctocodeMode /^⚙️  Mode:.*$/
        syntax match OctocodeSeparator /^[═─━]*$/
        syntax match OctocodeHeader /^═══.*═══$/
        syntax match OctocodeFile /\d\+\.\s\+[^:]\+:\d\+-\d\+/
        syntax match OctocodeSymbol /Symbols:.*$/
        syntax match OctocodePreview /^\s\{5,\}.*$/
        syntax match OctocodeInstruction /^[•▶].*$/
        syntax match OctocodeEmoji /[🔍⚙️📄📚📝🎯]/
      ]])
    end)
  end
end
...
local function update_display(query, results_lines)
  ...
  -- Beautiful input section with mode integrated
  ...
  -- Results section with beautiful formatting
  if results_lines and #results_lines > 0 then
    ...
    table.insert(lines, "")
    table.insert(lines, "▶ Ready to search! Type your query above and press <Esc>")
    table.insert(lines, "")
    table.insert(lines, "🎯 Quick Commands:")
    table.insert(lines, "  • gi - Jump to input and start typing")
    table.insert(lines, "  • gr - Jump to results section")
    table.insert(lines, "  • dd - Clear search query (on input line)")
    table.insert(lines, "  • <Enter> - Open file in right panel")
    table.insert(lines, "  • q - Close search panel")
    table.insert(lines, "  • ? - Show help")
    table.insert(lines, "")
    table.insert(lines, "🔧 Mode Switching:")
    table.insert(lines, "  • ma - All content")
    table.insert(lines, "  • mc - Code only")
    table.insert(lines, "  • md - Documentation")
    table.insert(lines, "  • mt - Text content")
    table.insert(lines, "")
    table.insert(lines, "✨ All vim commands work: 9j, gg, G, /search, etc.")
  end
  ...
  state.header_line = 1
  state.input_line = 2
  state.separator_line = 4
  state.results_start_line = 6
end
